...
public class EventResource {
    ...
    @RequestBody(
        description = "CloudEvent to store",
        required = true,
        content = @Content(schema = @Schema(implementation = CloudEventDTO.class))
    )
    @APIResponses({
    ...
    public Uni<Response> createEvent(@Valid CloudEventDTO event) {
        return eventService.storeEvent(event)
            .map(result -> {
                ...
                return Response.created(URI.create("/events/" + result.event().id))
                    .entity(result.event())
                    .build();
            });
    }
